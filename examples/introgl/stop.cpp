...
void Displaydot ( void ) {
	...
	for (int i = 0; i < 7; i++)
	{
		...
		glBegin(GL_LINES);
			glVertex2d(450 + vertices[i][0], 450 + vertices[i][1]);
			glVertex2d(450 + vertices[i+1][0], 450 + vertices[i+1][1]);
		glEnd();
	}

	glBegin(GL_LINES);
		glVertex2d(450 + vertices[0][0], 450 + vertices[0][1]);
		glVertex2d(450 + vertices[7][0], 450 + vertices[7][1]);
	glEnd();
    glFlush();
}
